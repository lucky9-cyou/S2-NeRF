...
def near_far_from_aabb_func(rays_o, rays_d, aabb, min_near=0.2):
    ...
    rd = 1 / rays_d
    near_x = (aabb[0] - rays_o[:, 0]) * rd[:, 0]
    far_x = (aabb[3] - rays_o[:, 0]) * rd[:, 0]

    # swap near and far if near is larger than far
    near_x, far_x = torch.min(near_x, far_x), torch.max(near_x, far_x)

    near_y = (aabb[1] - rays_o[:, 1]) * rd[:, 1]
    far_y = (aabb[4] - rays_o[:, 1]) * rd[:, 1]

    # swap near and far if near is larger than far
    near_y, far_y = torch.min(near_y, far_y), torch.max(near_y, far_y)

    near_x[torch.logical_or(near_x > far_y, near_y > far_x)] = 10

    near_x = torch.max(near_y, near_x)
    far_x = torch.min(far_y, far_x)

    near_z = (aabb[2] - rays_o[:, 2]) * rd[:, 2]
    far_z = (aabb[5] - rays_o[:, 2]) * rd[:, 2]

    # swap near and far if near is larger than far
    near_z, far_z = torch.min(near_z, far_z), torch.max(near_z, far_z)

    near_x[torch.logical_or(near_x > far_z, near_z > far_x)] = 10

    near_x = torch.max(near_z, near_x)
    far_x = torch.min(far_z, far_x)

    near_x = torch.max(
        torch.tensor(min_near, device=rays_d.device).expand_as(near_x), near_x)
...
class _march_rays(Function):
    ...
    def forward(ctx,
        ...
        _backend.march_rays(n_alive, n_step, rays_alive, rays_t, rays_o,
                            rays_d, bound, dt_gamma, max_steps, C, H,
                            density_bitfield, near, far, xyzs, dirs, deltas,
                            noises)
...
class _composite_rays(Function):
    ...
    def forward(ctx,
        ...
        _backend.composite_rays(n_alive, n_step, T_thresh, rays_alive, rays_t,
                                sigmas, rgbs, deltas, weights_sum, depth,
                                image)
        return tuple()
